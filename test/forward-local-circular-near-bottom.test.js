...
test("forward span=1 circular near bottom: autosearch→search ● chooses bottom (height=700 fixture)", () => {
  ...
  const l = chart.graph.links.find(
    (x) => x.source?.name === "autosearch" && x.target?.name === "search ●"
  );
  assert.ok(l, "Missing link autosearch → search ●");
  assert.ok(l.circular, "Expected autosearch → search ● to be circular in this fixture");
  assert.equal(
    l.circularLinkType,
    "bottom",
    `Expected autosearch → search ● to route bottom at height=700; got ${l.circularLinkType}`
  );
});
